...
<html lang="en">
  <head>
    ...
    <title>AlumniDocs |||Manage Notice</title>
    <!-- plugins:css -->
    ...
    if ($query->rowCount() > 0) {
        foreach ($results as $row) {
            ...
            <tr>
                ...
                <td>
                    <a href="view-single-noticeAd.php?viewid=<?php echo htmlentities($row->ID); ?>" class="btn btn-primary btn-xs">View</a>
                    <a href="manage-notice.php?delid=<?php echo htmlentities($row->ID); ?>" onclick="return confirm('Do you really want to delete this notice?');" class="btn btn-danger btn-xs">Delete</a>
                </td>
            </tr>
            <?php
            if (!empty($row->NoticeImage)) {
                echo "Image Path: " . htmlentities($row->NoticeImage); // Debugging
            }
            $cnt++;
        }
    } else {
