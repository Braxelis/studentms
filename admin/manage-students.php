...
<html lang="en">
  <head>
    ...
    <title>AlumniDocs||Manage Students</title>
    <!-- plugins:css -->
                      ...
                      <table class="table">
                        <thead>
                          <tr>
                            ...
                            <th class="font-weight-bold">Group Description</th>
...
$sql = "SELECT 
        ...
        FROM 
            tblstudent
        LEFT JOIN 
            group_students ON tblstudent.ID = group_students.student_id
        LEFT JOIN 
            groups ON group_students.group_id = groups.id
        LEFT JOIN 
...
if ($query->rowCount() > 0) {
    foreach ($results as $row) {
        ...
        <tr>
            ...
            <td><?php echo htmlentities($row->GroupName) ? htmlentities($row->GroupName) : 'N/A'; ?></td>
            <td>
                <a href="edit-student-detail.php?editid=<?php echo htmlentities($row->sid); ?>" class="btn btn-info btn-xs" target="blank">Edit</a>
                <a href="manage-students.php?delid=<?php echo htmlentities($row->sid); ?>" onclick="return confirm('Do you really want to delete this student?');" class="btn btn-danger btn-xs">Delete</a>
            </td>
        </tr>
        <?php
        $cnt++;
